...
export const fetchData = async ({ queryKey }: any) => {
  ...
  try {
    const data = await Api().get(`${apiUrl}?pageSize=${pagenation?.pageSize}&pageNumber=${pagenation?.current}&from=${pagenation?.from}&to=${pagenation?.to}&${pagenation?.selectedValue}=${pagenation?.searchValue}`);
...
// Update postData function to accept additional parameters
export const postData = async (payload: any, apiUrl: string) => {
  try {
    const data = await Api().post(`${apiUrl}`, payload);
    if ([200, 201].includes(data.status)) {
      return data?.data?.data.content;
