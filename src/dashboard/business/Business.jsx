...
export default function Dashboard() {
  ...
  const { data: responseData, isLoading: status, isError, } = useQuery(
    ["fetchBusinesses", tableParams?.pagination, "/admin/business"],
    fetchData, {
    onError: (err) => {
      alert(err);
    },
  ...
  // useEffect(() => {
  //   const myResponse = fetchMyBusinesses();
  ...
  const columns = [
    ...
    },
    {
      ...
      ...getColumnSearchProps('businessCategory'),
    },
    {
      title: 'Currency',
      dataIndex: 'currency',
      ...
      render: (_, record) => (
        <Space size="middle">
          <Tooltip title="View User">
            <Button
              type="primary"
              onClick={() => { setOpen(true); setCardData(record); navigate(`/single-business?id=${record.id}`); localStorage.setItem("SingleBusiness", JSON.stringify(record)); }}
              icon={<SearchOutlined />}
              size="small"
              style={{
                width: 80,
  ...
  const popoverContentMenu = (
    <div>
      <Card>
        <div className='flex'>
          <CSVLink filename={"Businesses-Huzz-file.csv"} data={data}>
            <Button style={{
              width: 90,
  ...
  return (
    <>
      <div className="bg-042-green pt-14 pb-28 px-3 md:px-8 h-auto">
      ...
      <div className='w-1/2 m-auto flex justify-center'>
        <CardHeader color="green" contentPosition="none">
          <div className="w-full flex items-center justify-between">
            <h2 className="text-white text-2xl">Huzz Businesses</h2>
            <div>
